...
def test_todo_panels_delegate_rendering_to_shared_helpers():
    ...
    # Verify stale-sync hash dedup is still present
    assert "_workspaceTodosLastRenderedHash" in workspace_block
    assert "_workspaceTodosHash(todos)" in workspace_block
    assert "if(hash === _workspaceTodosLastRenderedHash) return;" in workspace_block
